      ...
      if (method_call.method_name().compare("printPdf") == 0)
      {
        ...
        auto usePrinterSettings = std::get<bool>(
            arguments->find(flutter::EncodableValue("usePrinterSettings"))
                ->second);
        auto printCopies = std::get<int>(
            arguments->find(flutter::EncodableValue("printCopies"))->second);

        auto showSystemPrintUI = std::get<bool>(
            arguments->find(flutter::EncodableValue("showSystemPrintUI"))->second);

        std::cout << "取出 copies: " << printCopies << std::endl;
        std::cout << "取出 showSystemPrintUI: " << showSystemPrintUI << std::endl;
        std::cout << "取出 showSystemPrintUI: " << showSystemPrintUI << std::endl;
        ...
        auto res =
            job->printPdf(name, printer, width, height, usePrinterSettings, showSystemPrintUI, printCopies);
        if (!res)
        {
          delete job;
        }
